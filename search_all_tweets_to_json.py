...
def tweet_search(api, max_tweets, max_id, since_id, geocode):
    ...
    while len(searched_tweets) < max_tweets:
        ...
        try:
            new_tweets = api.search(q='*',count=remaining_tweets,
                                    since_id=str(since_id),lang='en',
				                    max_id=str(max_id))
#                                    geocode=geocode)
...
def main():
    ...
    ''' search variables: '''
    time_limit = 0.002                       # runtime limit in hours
    max_tweets = 100                           # number of tweets per search (will be
                                               # iterated over) - maximum is 100
    min_days_old, max_days_old = 4,5      # search limits e.g., from 7 to 8
    ...
    json_file_root =  "All_Tweets"+ '/'  + "All_Tweets"
    ...
    if max_days_old - min_days_old == 1:
        ...
        day = '{0}-{1:0>2}-{2:0>2}-{3:0>2}-{4:0>2}'.format(d.year, d.month, d.day,d.hour,d.minute)
    else:
        d1 = dt.datetime.now() - dt.timedelta(days=max_days_old-1)
        d2 = dt.datetime.now() - dt.timedelta(days=min_days_old)
        day = '{0}-{1:0>2}-{2:0>2}-{3:0>2}{4:0>2}_to_{5:0>2}-{6:0>2}-{7:0>2}-{8:0>2}{9:0>2}'.format(
